...
想必你应该很清楚如何使用express搭建服务器了，下面是TechNode的服务器：
f
```
...
我们将静态文件放在`staitc`目录下，包括`index.html`，我们不适用`jade`来生成html页面。
...
接下来就是建立服务端与浏览器端的socket服务。socket.io的API是基于事件的，我们监听着`connection`事件，如果有客户端链接上来，我们就可以或者一个socket对象，这样我们就可以和这个客户端实时通信了。
